...
{
    public class AppointmentPrescriptionTests : IClassFixture<TestingHospitalFactory<Startup>>
    {
        private readonly HttpClient _client;
        private readonly TestingHospitalFactory<Startup> _factory;

        public AppointmentPrescriptionTests(TestingHospitalFactory<Startup> factory)
        {
            _factory = factory;
        ...
        {
            var getRequest = new HttpRequestMessage(HttpMethod.Get, "api/appointment/prescription/-1");
        ...
        {
            var getRequest = new HttpRequestMessage(HttpMethod.Get, "api/appointment/prescription/-1485");
            getRequest.Headers.Authorization = new AuthenticationHeaderValue("Bearer", await GetToken());
            var response = await _client.SendAsync(getRequest);
