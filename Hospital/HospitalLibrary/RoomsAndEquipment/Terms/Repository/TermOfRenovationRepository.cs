...
namespace HospitalLibrary.RoomsAndEquipment.Terms.Repository
{
    public class TermOfRenovationRepository : GenericDbRepository<TermOfRenovation>, ITermOfRenovationRepository
    {
        ...
        {
            return _dbContext.TermOfRenovations.Where(t => t.StateOfRenovation == StateOfTerm.PENDING && t.TimeInterval.EndTime <= DateTime.Now && t.TimeInterval.EndTime >= DateTime.Now.AddMinutes(-2)).ToList();
        }
    }
}
